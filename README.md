...
Public controllers should use HTTPS. The currently configured deployment at `http://45.50.0.74:8765` is explicitly trusted by the Windows installer so one-click setup works with the existing controller, but its HTTP transport is plaintext. Other public HTTP controller URLs are still refused unless `-AllowInsecurePublicController` is passed explicitly.
