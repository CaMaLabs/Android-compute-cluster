...
<main>
 ...
 <form id="form"><textarea id="prompt" placeholder="Ask the agent to inspect the host, manage services, use Ollama, submit swarm work, or use Composio tools..."></textarea><button id="voice" class="secondary" type="button">Voice</button><button id="send" type="submit">Send</button></form>
	...
	const $=id=>document.getElementById(id);let messages=[];let recognition=null;let listening=false;const trustedVoiceUrl='https://putting-mapping-mart-hypothesis.trycloudflare.com/agent';
	let head=null;let avatarReady=false;let avatarBusy=false;let handTimer=null;let mouthTimer=null;let mouthOpen=false;
	...
	function setMouth(amount){if(!head||!head.armature)return;const weights={jawOpen:.7,mouthOpen:.55,viseme_aa:.6,viseme_E:.22,viseme_O:.18,viseme_U:.14};head.armature.traverse(o=>{if(!o.morphTargetDictionary||!o.morphTargetInfluences)return;for(const [name,weight] of Object.entries(weights)){const i=o.morphTargetDictionary[name];if(i!==undefined)o.morphTargetInfluences[i]=amount*weight}})}
	function startMouth(){clearInterval(mouthTimer);mouthOpen=false;mouthTimer=setInterval(()=>{mouthOpen=!mouthOpen;setMouth(mouthOpen?(0.45+Math.random()*0.45):(Math.random()*0.12))},95)}
